...
class AnimSprite {
    ...
    draw(vel, dead) {
        ...
        if (dead) {
            image(this.source.get(16 * 6, 0, 16, 16), this.x, this.y, this.w, this.h)
...
class Enemy {
    ...
    draw() {
        imageMode(CENTER)
        if (this.speed == 0) {
            image(blocks[this.sprite].get(2 * 16, 0, 16, 16), floor(this.x), floor(this.y), 16, 16)
            this.h = 0
            this.w = 0
            return
        }
        // console.log((floor(this.index) % 2))
        image(blocks[this.sprite].get((floor(this.index) % 2) * 16, 0, 16, 16), floor(this.x), floor(this.y), this.w, this.h)
        this.update()
    }
